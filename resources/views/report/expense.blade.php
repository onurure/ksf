        ...
        <div class="widget-body">
            <div class="row">
                <div class="col-sm-12">
                    ...
                    <a class="btn btn-primary" href="{{url('expensereport')}}">Borç Raporu</a>
                    ...
                    <div class="carousel-wrap">
                        <div class="owl-carousel owl-theme">
                            @foreach ($firms as $firm)
                                <a class="{{$firm->id == request()->firm ? 'selected' : ''}} item link" href="{{ url('expensereport?firm='.$firm->id) }}"><img src="{{ url($firm->logo) }}"></a>
                            @endforeach
                        </div>
                    </div>
                </div>
                ...
                <div class="row">
                    <div class="col-sm-4">
                        <div class="form-group">
                            <label for="email">Kime:</label>
                            <select class="js-select2 form-control" multiple="multiple" name="kime[]">
                                @foreach($kime as $project)
                                    <option value="{{$project}}" data-badge="" {{ isset(request()->projects) && in_array($projects,request()->projects) ? 'selected' : '' }}>{{$project}}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="form-group">
                            <label for="email">Borç Tipi:</label>
                            <select class="js-select2 form-control" multiple="multiple" id="" name="expensetip[]">
                                @foreach($expenses as $expense)
                                    <option value="{{$expense->id}}" data-badge="" >{{$expense->name}}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
    ...
    <script>
        ...
        function ara(){
            ...
            "language": {
                ...
                "ordering": false,
                "ajax":{
                    url :"{{url('expensereport')}}", // json datasource
                    type: "post",
                    dataType: "json",
                    data: function(d) {
                        var o = {};
